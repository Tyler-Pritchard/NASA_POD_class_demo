...
def show_photos():
    ...
    photo_data = connection.fetch_photo_data()
    for i in range(len(photo_data)):
        if photo_data[i]['media_type'] == 'image':
            try:
                photo = AstronomyPhoto()
                photo.photo_url = photo_data[i]['url']
                photo.date = photo_data[i]['date']
                photo.title = photo_data[i]['title']
                print(photo.date)
